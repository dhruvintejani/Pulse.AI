  ...
  return (
    <div className="h-full overflow-y-auto no-scrollbar">
      <div className="max-w-5xl mx-auto p-6 pb-32 space-y-6">
              ...
              return (
                <motion.div key={stat.label} whileHover={{ y: -3, scale: 1.01 }} className="bg-[#FFFDF8] rounded-2xl p-4 border border-[rgba(0,0,0,0.05)] shadow-card">
                  <div className="flex items-start justify-between mb-3">
                    <div className="w-9 h-9 rounded-xl bg-[rgba(233,162,76,0.1)] flex items-center justify-center"><Icon size={16} className="text-[#E9A24C]" /></div>
          ...
          <div className="bg-[#FFFDF8] rounded-2xl p-5 border border-[rgba(0,0,0,0.05)] shadow-card">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              {quickActions.map((action) => {
                ...
                return (
                  <motion.button key={action.label} whileHover={{ y: -4, scale: 1.02 }} whileTap={{ scale: 0.97 }} onClick={() => navigate(action.path)} className={`bg-gradient-to-br ${action.color} rounded-xl p-4 text-left border border-white/80 hover:shadow-card transition-shadow duration-200`}>
                    <div className="w-9 h-9 rounded-xl bg-white/70 flex items-center justify-center mb-3"><Icon size={17} className={action.iconColor} /></div>
        ...
        <div className="grid lg:grid-cols-3 gap-5">
          <FadeIn delay={0.15} className="lg:col-span-2">
            <div className="bg-[#FFFDF8] rounded-2xl p-5 border border-[rgba(0,0,0,0.05)] shadow-card h-full">
              <div className="flex items-center justify-between mb-4">
                ...
                <button className="text-xs text-[#E9A24C] font-semibold hover:underline flex items-center gap-1">View all <ChevronRight size={12} /></button>
              </div>
              <div className="space-y-2">
                ...
                {!dashboardQuery.isLoading && dashboardData.activity.map((item) => (
                  <motion.div key={item.id} whileHover={{ x: 3 }} onClick={() => navigate(item.type === 'chat' ? '/dashboard/chat' : item.type === 'doc' ? '/dashboard/documents' : '/dashboard/workspace')} className="flex items-center gap-3.5 p-3 rounded-xl hover:bg-[rgba(233,162,76,0.05)] cursor-pointer transition-all duration-150 group">
                    <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${item.type === 'chat' ? 'bg-[rgba(233,162,76,0.12)]' : item.type === 'doc' ? 'bg-blue-50' : 'bg-purple-50'}`}>
                      {item.type === 'chat' ? <MessageSquare size={15} className="text-[#E9A24C]" /> : item.type === 'doc' ? <FileText size={15} className="text-blue-500" /> : <Zap size={15} className="text-purple-500" />}
                    </div>
                ...
                {!dashboardQuery.isLoading && dashboardData.activity.length === 0 && <p className="text-sm text-[#999] text-center py-8">No activity yet.</p>}
              </div>
            </div>
          </FadeIn>
          ...
          <FadeIn delay={0.2} className="space-y-5">
            <div className="bg-[#1F1F1F] rounded-2xl p-5 text-white">
              <div className="flex items-center gap-2 mb-4"><Brain size={16} className="text-[#E9A24C]" /><h2 className="text-sm font-bold">Active Model</h2></div>
              <div className="space-y-2">
                {[
                  ...
                  <button key={model.name} onClick={() => setActiveModel(model.name)} className={`w-full flex items-center justify-between p-2.5 rounded-xl cursor-pointer transition-all ${activeModel === model.name ? 'bg-[rgba(233,162,76,0.15)] border border-[rgba(233,162,76,0.3)]' : 'hover:bg-white/5'}`}>
                    <div className="flex items-center gap-2.5"><div className={`w-2 h-2 rounded-full ${activeModel === model.name ? 'bg-[#E9A24C]' : 'bg-white/20'}`} /><span className="text-sm font-medium">{model.name}</span></div>
              ...
              <div className="space-y-3">
                {[
                  ...
                  <div key={u.label}>
                    ...
                    <div className="h-1.5 rounded-full bg-[rgba(0,0,0,0.06)]"><motion.div initial={{ width: 0 }} animate={{ width: `${(u.used / u.total) * 100}%` }} transition={{ duration: 1 }} className="h-full rounded-full" style={{ background: u.color }} /></div>
                  </div>
                ))}
              </div>
            </div>

            <motion.div whileHover={{ scale: 1.01 }} className="bg-gradient-to-br from-[#E9A24C] to-[#D4853A] rounded-2xl p-5 text-white cursor-pointer" onClick={() => navigate('/dashboard/billing')}>
              <div className="flex items-center gap-2 mb-2"><Star size={15} className="text-white/80" /><span className="text-xs font-semibold uppercase tracking-wider text-white/80">Upgrade</span></div>
              <p className="text-sm font-bold mb-3">Unlock unlimited AI access</p>
              <Button variant="secondary" size="sm" className="w-full">View Enterprise plans <ArrowRight size={13} className="ml-1" /></Button>
            </motion.div>
          </FadeIn>
        </div>
        ...
        <FadeIn delay={0.25}>
          <div className="bg-[#FFFDF8] rounded-2xl p-5 border border-[rgba(0,0,0,0.05)] shadow-card">
            <div className="flex items-center gap-2 mb-4"><Sparkles size={15} className="text-[#E9A24C]" /><h2 className="text-sm font-bold text-[#1F1F1F]">Suggested for you</h2></div>
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-2.5">
              {['Summarize my latest documents', 'Draft a product announcement', 'Research competitor landscape', 'Generate 10 blog post ideas', 'Analyze my sales data trends', 'Write a technical blog post'].map((prompt) => (
                <motion.button key={prompt} whileHover={{ scale: 1.01, x: 2 }} whileTap={{ scale: 0.98 }} onClick={() => navigate('/dashboard/chat')} className="text-left text-sm text-[#444] px-4 py-3 rounded-xl border border-[rgba(0,0,0,0.06)] hover:border-[rgba(233,162,76,0.3)] hover:bg-[rgba(233,162,76,0.04)] transition-all duration-150 font-medium">
                  {prompt}
                </motion.button>
              ))}
            </div>
